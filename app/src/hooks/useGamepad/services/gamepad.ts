import { genericButtonsId, xboxButtonsId } from './gamepadButtons';
...
const buttonLoop = (gamepadIndex: number, onPress: App.Gamepad.OnButtomPressed) => {
  ...
  return setInterval(() => {
    ...
    axes.forEach((axis, i) => {
      ...
      if (![6, 7].includes(i)) return false;
      const axisId = `axis${i}${axis > 0 ? 'Pos' : 'Neg'}`;
      if (genericButtonsId[axisId]) pressedButtons.push(genericButtonsId[axisId]);
    });

    buttons.forEach((Button, i) => {
      if (Button.value === 0) return false;
      const buttonId = `button${i}`;
      if (genericButtonsId[buttonId]) pressedButtons.push(genericButtonsId[buttonId]);
    });
...
export const onDisconnected = (ev: GamepadEvent) => {
  ...
  buttonLoopInterval.forEach(int => clearInterval(int as unknown as number));
};
